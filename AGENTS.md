...
- `lua/plantuml/` modules:
  ...
  - `viewport.lua` — pure crop/pan math for the zoomed viewport plus the async ImageMagick `magick -crop` call.
...
- **Each render writes to its own directory** `<temp_dir>/<bufnr>_<hrtime>/`, producing `<bufnr>.png` / `<bufnr>_<n>.png` inside (multi-diagram files). `renderer.find_output_files` parses the source buffer's `@startuml` blocks and maps them to PlantUML's output filenames **in source order**: unnamed diagrams get `<bufnr>.png` / `<bufnr>_%03d.png`, named diagrams (`@startuml Name`) get `Name.png`. It returns parallel `paths, names` arrays; unmatched leftover files are appended sorted. Render dirs are only deleted on `preview.close` (`renderer.cleanup`); never delete them while a preview is open, or image.nvim will fail with "unable to open image" when it re-identifies a deleted file.
...
- Zoom above `zoom=1` switches the preview to a **viewport**: a window-sized, magnified crop of the source diagram (via ImageMagick, already a dependency of image.nvim's default `magick_cli` processor). Crop math lives in `lua/plantuml/viewport.lua` (pure, unit-tested); the crop itself is async (`vim.system magick -crop`) with a per-preview `view_gen` generation guard. `zoom=1` means "fit the whole diagram inside the preview window" (aspect preserved); zoom < 1 shrinks, zoom > 1 magnifies a crop. Zooming keeps the viewport center fixed; pan (keyboard `pan_*` or mouse drag via `<LeftMouse>` + a polling `uv.new_timer`) moves the focused region. **Cropped viewport files live in `<temp_dir>/<bufnr>_view_<hrtime>/` and are only deleted on `preview.close`** — same never-delete-while-image.nvim-references-it rule as render dirs. The preview window's statusline shows `[index/total] <name> (<zoom>x)`; zoom config lives in `config.zoom` (`step`/`min`/`max`/`pan_step`).
